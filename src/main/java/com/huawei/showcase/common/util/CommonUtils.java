...
public final class CommonUtils
{
  ...
  public static <V> boolean checkCollectionNullOrEmpty(Collection<V> list)
  {
	...
	public static String createTransactoinId(String userName) {
		...
		if (!checkAllStringNull(new String[] { userName })) {
	...
	public static String getUserFarmId(String userId, String farmId) {
		String id = userId;
		if (checkAllStringNull(new String[] { farmId })) {
			if (checkAllStringNull(new String[] { userId }))
				;
		} else {
			id = id + farmId;
			return id;
	...
	public static boolean checkRandomId(String req, String rsp) {
		if (!checkAllStringNull(new String[] { req })) {
			if (!checkAllStringNull(new String[] { rsp }))
				;
		} else {
			return true;
		}
		if (!req.equalsIgnoreCase(rsp)) {
			LogUtils.VDESKTOP_LOG.error("randomId is error. req = " + req
					+ " rsp = " + rsp);
			return false;
		}
		return true;
